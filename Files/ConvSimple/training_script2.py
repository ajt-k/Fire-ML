...
class RandomCropWithNonZeroCheck:
    def __init__(self, size, min_non_zero):
        self.size = size
        self.min_non_zero = min_non_zero
    ...
    def __call__(self, img):
        ...
        img_gray = transforms.functional.to_grayscale(img)  # Convert to grayscale
        img_gray_tensor = img_gray.unsqueeze(0)  # Add batch dimension

        for _ in range(10):  # Try up to 10 times to find a crop that meets the requirement
            top = random.randint(0, h - crop_h)
            left = random.randint(0, w - crop_w)
            crop = img_gray_tensor[:, top:top + crop_h, left:left + crop_w]

            # Count the number of non-zero pixels across all channels
            non_zero_pixels = torch.sum(crop > 0).item()

            if non_zero_pixels >= self.min_non_zero_pixels:
                # Perform the crop on the original image tensor
                return img[:, top:top + crop_h, left:left + crop_w]

        # If no satisfactory crop is found, return a central crop as a fallback
